...
const ContactItem: React.FC<ContactItemProps> = ({ contact, index, onEdit, onDelete }) => {
  return (
    <>
      <tr key={contact.id} className={`${index}`} onClick={() => onEdit(contact)}>
          ...
          <div className="contact-info">
            ...
            <div>
              <div className="contact-name">
                {formatField(contact.firstName, 15)} {formatField(contact.lastName, 15)}
              </div>
        ...
        <td>
          <span className={`status-badge status-${contact.status}`}>
            {formatField(contact.status ? contact.status : "New", 10)}
          </span>
        </td>
        ...
        <td>
          ...
          {contact!.status!.trim().toLowerCase() === "new" &&
            <button className="action-btn btn-delete" onClick={(e) => { e.stopPropagation(); }}>
